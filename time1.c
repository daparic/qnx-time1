...
struct
{
    rcvid_t rcvid;                  // receive ID of client
...
static  void gotAMessage (rcvid_t rcvid, ClientMessageT *msg);

int
main (void)                         // ignore command-line arguments
{
    rcvid_t rcvid;                  // receive ID of the sender
...
void
gotAMessage (rcvid_t rcvid, ClientMessageT *msg)
{
    int i;
    ...
    case    MT_WAIT_DATA:
        ...
        for (i = 0; i < MAX_CLIENT; i++) {
            ...
            if (!clients [i].in_use) {
                ...
                clients [i].timeout = 60;
                return;
            }
        }

        fprintf (stderr, "Table full, message from rcvid %ld ignored, "
                         "client blocked\n", rcvid);
        break;
    ...
    case    MT_SEND_DATA:
        ...
        // value 42 is the shutdown sentinel
        if (msg -> messageData == 42) {
            printf ("%s:  received shutdown sentinel (42), terminating\n",
                    progname);
        ...
        fprintf (stderr, "Table empty, message from rcvid %ld ignored, "
                         "client blocked\n", rcvid);
        break;
    }
}
